...
import { hydroKpiService, HydroKpiSnapshot } from '../../services/hydroKpiService';

import { notificationService } from '../../services/notificationService';

import { EmptyState } from '../Shared/EmptyState';

import { firstDayOfMonthISO, formatDateBR, lastDayOfMonthISO } from '../../utils/dateUtils';
...
export const HydroSysAnalytics: React.FC<{ user: User }> = ({ user }) => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            const kpiSnapshot = hydroKpiService.buildSnapshot({
                certificados,
                filtros,
                cloro,
            ...
            setSedeNameById(sedeMap);
            setSnapshot(kpiSnapshot);
            setLoading(false);
        };
    ...
    const expiredCount = snapshot.expiredCount;
    const warningCount = snapshot.warningCount;
                                            ...
                                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                                ...
                                                {criticalItems.map((item, idx) => (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-4 py-4 text-xs font-black uppercase tracking-wider text-cyan-700 dark:text-cyan-300">{sedeNameById[item.sedeId] || item.sedeId}</td>
