...
import { ArrowLeft, Search, Plus, Trash2, Edit2, Shield, X, User as UserIcon, Building, Key, Copy, Check, Save } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { User, UserRole, UserStatus, Sede } from '../types';
...
export const AdminUserManagement: React.FC = () => {
  ...
  // FIXED: Changed dependency from [currentUser] (object) to [currentUser?.id] (primitive)
  // to prevent infinite re-renders loop which was freezing the UI.
  useEffect(() => {
  ...
  const loadData = () => {
    ...
    // Permission Logic: Gestor sees only users from their Sede
    if (currentUser?.role === UserRole.GESTOR) {
        allUsers = allUsers.filter(u => u.sedeId === currentUser.sedeId);
    }
  ...
  const handleStartNew = () => {
    ...
    // Pre-fill Sede if Gestor
    const defaultSede = currentUser?.role === UserRole.GESTOR ? currentUser.sedeId : '';
    
    setFormData({ ...initialFormState, sedeId: defaultSede });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing && editingId) {
        ...
        // CREATE
        // If Gestor creating, force Sede ID just in case
        if (currentUser?.role === UserRole.GESTOR) {
            formData.sedeId = currentUser.sedeId;
        }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
             {currentUser?.role === UserRole.ADMIN ? 'Controle total do sistema.' : `Gerenciando unidade: ${sedes.find(s => s.id === currentUser?.sedeId)?.name || 'Minha Unidade'}`}
          </p>
        </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Sede</th>
              ...
              {filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">
                      {sedes.find(s => s.id === user.sedeId)?.name || '-'}
                  </td>
                  <td className="px-6 py-4">
                    <button 
      ...
      {isModalOpen && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95">
                  ...
                  {createdUserPass ? (
                      // SHOW PASSWORD SCREEN (Only after Creation)
                      <div className="p-8 text-center bg-emerald-50/50 dark:bg-emerald-900/10">
                          ...
                          <p className="text-slate-600 dark:text-slate-400 mb-6 text-sm">
                              ...
                              <span className="font-bold text-red-500">Copie e envie para o usuário agora</span>, ela não poderá ser vista novamente.
                          </p>
                          
                          <div className="bg-white dark:bg-slate-950 p-4 rounded-xl border-2 border-dashed border-emerald-200 dark:border-emerald-800 flex items-center justify-between mb-8 group hover:border-emerald-400 transition-colors">
                              ...
                              <button 
                                ...
                                className="p-2 bg-slate-100 dark:bg-slate-800 hover:bg-emerald-100 dark:hover:bg-emerald-900 text-slate-600 dark:text-slate-400 hover:text-emerald-700 rounded-lg transition-colors flex items-center gap-2 text-sm font-bold"
                              >
                                  <Copy size={18} /> Copiar
                              </button>
                          </div>

                          <button onClick={closeAndReset} className="w-full py-3 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl shadow-lg shadow-brand-500/30 transition-transform active:scale-95">
                              Entendido, Concluir.
                          </button>
                      </div>
                  ) : (
                      ...
                      <form onSubmit={handleSubmit} className="p-6 space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                              <div>
                                  ...
                                  <select 
                                      ...
                                      // Gestors can only create Operational or Gestor, not Admins
                                      disabled={currentUser?.role !== UserRole.ADMIN && isEditing}
                                  >
                                      {Object.values(UserRole).map(role => (
                                          // Filter: If I'm a Gestor, I shouldn't see/select ADMIN
                          ...
                          <div>
                              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Sede de Acesso</label>
                              <div className="relative">
                                  ...
                                  <select 
                                      ...
                                      // Gestors cannot change the Sede, it's fixed to theirs
                                      disabled={currentUser?.role === UserRole.GESTOR}
                                  >
                                      <option value="">Acesso Global (Sem Sede Específica)</option>
                                      {sedes.map(sede => (
